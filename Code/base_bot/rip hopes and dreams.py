...
class AprilTag:
    ...
    def update_action(self, status, x, y, dist_in):
        ...
        if status == 'STARTUP':
            ...
            Constants.MAX_CAM_X_BOUND + 10 >= x >= Constants.MIN_CAM_X_BOUND - 10) and \
            ...
            if (x <= Constants.MAX_CAM_X_BOUND or x <= Constants.MAX_CAM_X_BOUND + 10) and \
                    (self.current_tag == self.left_tag or self.current_tag == self.right_tag):
    ...
    def splitTagsSeen(self, tag_data, num_of_tags):
	...
	parsed_tag_data = self.splitTagsSeen(temp_tag_data, num_tags)
	...
	for i in range(len(parsed_tag_data)):
		...
		if self.status == 'CREEP_FORWARD' and (current_tag[0] == self.right_tag):
